...
- The ARM fighter registry now has an independently backed Falco row at the reference mod's fkind 29. Falco's fighter data is copied before its file IDs and motion pointers are changed, preserving the live vanilla Fox row. The file-size and animation-heap prepass scans registered rows through fkind 127 and skips unused IDs. Falco can now be chosen in VS mode by selecting Fox on the original top-screen grid and tapping that player's bottom-screen card. A private Azahar harness used that touch path in a timed four-fighter stereo match, observed 46 action states, reached results, and returned to VS select without an invalid memory access. Separate match inspections confirmed vanilla Fox loads main file `0xD1` and tapped Falco loads `0x8AB`. This is an initial selection bridge, not the expanded Remix character-select interface.
