...
- A reference-table audit now identifies all 88 added fighter records, including the 19 +EXTRA records. It resolves original menu-script references to their native decomp symbols, decodes the expanded custom-command formats, and traces each fighter's asset dependencies. It can generate ARM-syntax-checked motion data for 69 records. Apart from Falco, DK Ult, and Japanese Pikachu, these are data imports only: the other fighters remain disabled until native callbacks, roster presentation, and gameplay are integrated.
...
- The results scene now loads the main files for registered extra fighters before constructing their result-screen models. On return to VS select, added fighters map back to their original parent portraits while their bottom cards preserve the selected imported fighter for the next match. DK Ult's and Japanese Pikachu's four-fighter stereo matches reached results and returned to select without an invalid memory access. Untapped Donkey Kong retained its original main file `0xD5` while DK Ult loaded `0x16CC`; untapped Pikachu retained `0xF3` while Japanese Pikachu loaded `0x987`.
- Kirby's inhale path now bounds-checks the original 27-row copy table. Inhaling Falco, DK Ult, or Japanese Pikachu uses the Fox, Donkey Kong, or vanilla Pikachu copy data until their unique copy abilities are ported; unrecognized added fighters safely fall back to no copy. The fix also covers Giant Donkey, whose fighter ID lies just past the original table.
...
Fox, Pikachu, and Donkey Kong remain available. In VS mode, their bottom-screen cards switch to Falco (fkind 29), Japanese Pikachu (fkind 50), and DK Ult (fkind 81). The top-screen portraits, announcer and some presentation still use the vanilla parents. Training and other modes have not gained this selector. Menus, stages, items, music, save layout, the remaining fighters and AI are inherited from the original port. Remix-wide engine changes, unique Kirby copy abilities, expanded selection screens and the other +EXTRA fighters are not implemented. This fixture establishes three native integration paths; it does not establish complete character parity or full-mod compatibility.

The Falco executable passed nine move checks in Azahar: grounded/airborne Phantasm, jab, down smash, reflector, Firebird, grab script/throw-pointer setup, taunt, and down aerial. DK Ult passed direct checks for jab, Giant Punch charge and speed, Spinning Kong, and aerial down special. Japanese Pikachu passed direct checks for jab, Thunder Jolt, and Quick Attack. Four-fighter stereo matches reached results and returned to select for all three imported fighters. These are smoke tests, not complete move/interactions coverage. Details and matching package hashes are in `remix/verification.json`. Real-console performance is unmeasured; debugger-instrumented timing is not hardware frame-rate evidence.
