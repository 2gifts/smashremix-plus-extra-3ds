...
- A reference-table audit now identifies all 88 added fighter records, including the 19 +EXTRA records. It resolves original menu-script references to their native decomp symbols, decodes the expanded custom-command formats, and traces each fighter's asset dependencies. It can generate ARM-syntax-checked motion data for 69 records. Apart from Falco and DK Ult, these are data imports only: the other fighters remain disabled until native callbacks, roster presentation, and gameplay are integrated.
...
- The results scene now loads the main files for registered extra fighters before constructing their result-screen models. On return to VS select, Falco and DK Ult map back to the original Fox and Donkey Kong portraits while their bottom cards preserve the selected imported fighter for the next match. DK Ult's four-fighter stereo match reached results and returned to select without an invalid memory access; untapped Donkey Kong retained his original main file `0xD5`, and DK Ult loaded `0x16CC`.
- Kirby's inhale path now bounds-checks the original 27-row copy table. Inhaling Falco or DK Ult uses the Fox or Donkey Kong copy data until their unique copy abilities are ported; unrecognized added fighters safely fall back to no copy. The fix also covers Giant Donkey, whose fighter ID lies just past the original table.
...
Fox and Donkey Kong remain available. In VS mode, their bottom-screen cards switch to Falco (fkind 29) and DK Ult (fkind 81). The top-screen portraits, announcer and some presentation still use the vanilla parents. Training and other modes have not gained this selector. Menus, stages, items, music, save layout, the remaining fighters and AI are inherited from the original port. Remix-wide engine changes, Kirby's Falco copy ability, expanded selection screens and the other +EXTRA fighters are not implemented. This fixture establishes two native integration paths; it does not establish complete character parity or full-mod compatibility.

The Falco executable passed nine move checks in Azahar: grounded/airborne Phantasm, jab, down smash, reflector, Firebird, grab script/throw-pointer setup, taunt, and down aerial. DK Ult passed direct checks for jab, Giant Punch charge and speed, Spinning Kong, and aerial down special. Four-fighter stereo matches reached results for both imported fighters. These are smoke tests, not complete move/interactions coverage. Details and matching package hashes are in `remix/verification.json`. Real-console performance is unmeasured; debugger-instrumented timing is not hardware frame-rate evidence.
