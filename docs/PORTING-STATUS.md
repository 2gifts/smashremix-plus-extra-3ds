...
- The ARM fighter registry now has an independently backed Falco row at the reference mod's fkind 29. The file-size and animation-heap prepass scans registered rows through fkind 127 and skips unused IDs. A private Azahar harness forced fkind 29 into a four-fighter stereo match, observed 33 action states, and reached results without an invalid memory access. The normal selection screen still offers only the vanilla roster; the shipped development fixture continues to select Falco through the Fox replacement.
