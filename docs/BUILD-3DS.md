...
`native_callback_dependencies.py` also emits the ignored `remix/build/native-callback-dependencies.json` during fixture preparation. It follows direct `jal` edges through the pinned expansion and ranks original engine calls and shared helpers by their unbound action-table uses. Run `remix/.venv/Scripts/python.exe remix/tools/native_callback_dependencies.py` to refresh it separately after preparing a fixture. Its first-return scan and unknown-control-flow markers make it a triage tool; indirect calls, branches beyond the scan window, function-pointer callbacks, and mod-wide engine hooks need further reverse engineering before any generated ARM binding is safe. The report does not execute MIPS or increase the playable roster.

`native_vanilla_clone_callbacks.py` begins translating the upstream `OS.copy_segment` pattern in bulk. It verifies the original Mario up-special map routine against the user's pinned US ROM and compares all 48 compiled words of every action callback. Only the `lui a2` landing-lag immediate may differ; non-finite or out-of-range values and any other code change remain unbound. One native map implementation and wrappers for the distinct compiled lag values replace 17 N64 targets across 61 action slots, including Falco's former hand-written callback. The generated source and manifest remain local; the repository contains only the importer. Other copied vanilla functions need their own checked native adapter before this mechanism can cover them.

`native_source_copy_inventory.py` scans the staged, pinned assembly source for active `OS.copy_segment` calls, joins enclosing scopes to final symbols, and ranks unbound action callbacks by all their table uses. The ignored `native-source-copy-inventory.json` currently links 46 callback scopes and 103 copy segments; 32 of those scopes remain unbound. This is a candidate list, not proof of equivalent native behavior. `native_air_drift_overrides.py` applies the first additional adapter: it checks Goemon's copied 16-word vanilla air-drift segment and the complete modified main and helper routines, extracts the speed cap from the helper's `lui` instruction, and generates a shared native physics callback for its 15 action-table uses. Its owners still have other unbound callbacks, so this path is host-tested and ARM-built but not directly emulator-tested.

The restricted animation-end decoder also models a post-status write of the constant `1` to a motion flag and an explicitly loaded finite start frame. It recognizes two more Meta Knight callback pairs from the compiled ROM, bringing checked animation-end wrappers to 15 of 35 candidates. The native generator preserves the decoded order of status change, optional animation playback and flag write; unknown branches, calls, flag values, and invalid frames remain unbound. These owners are outside the current development roster, so the host tests and ARM build do not establish their gameplay behavior.
