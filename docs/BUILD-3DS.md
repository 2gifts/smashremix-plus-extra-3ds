...
For a validated fighter, add one record to [`remix/native_fighters.json`](../remix/native_fighters.json), with its reference fighter ID, vanilla parent, and `registration` set to `generic` when vanilla callbacks suffice. This catalog is a release gate after shared patch families and move behavior are implemented and emulator-tested, rather than a hand-written implementation for each fighter. Keep `custom` for fighters with their own native registration or status table. Run:
