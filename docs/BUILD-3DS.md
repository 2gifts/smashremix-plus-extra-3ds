...
For repeated mod callbacks, `classify_action_callbacks.py` groups compiled instruction shapes and their users. `native_transition_templates.py` checks collision wrappers and translates recognized status changes, symbol-bounded status tables, and fighter-kind branches into shared native C. `native_anim_end_templates.py` checks both the animation-end wrapper and its transition before generating native code. The generated manifests and `native-callback-coverage.json` show exactly which compiled addresses are bound. Extend a checked family only after confirming its complete MIPS side effects; keep unknown routines unbound. Fighter registration then consumes the resulting bindings automatically.
