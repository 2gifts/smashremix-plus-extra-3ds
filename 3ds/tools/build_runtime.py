...
def main():
    ...
    if probe:
        ...
        sources.append(ROOT/'src/remix_stages.c')
    if args.render:
    ...
    for src in sources:
        obj=out/(src.stem+'.o')
        if src.stem in ['game_host','vanilla_policy','render_bridge','gfx_pc','save_layout_check','stereo_camera','bottom_game','wallpaper','control_game','remix_falco_probe','remix_dkult_probe','remix_jpika_probe','remix_variants','remix_stages']:
            flags=game_flags()
        else:
