...
static unsigned fighterArt(unsigned c){return c==29?1:c==81?2:c==44?2:c==50?9:c==45?9:c==51?3:c==35?5:c==39?5:c==49?6:c==42?0:c==40?7:c==43?4:c==13?0:c==26?2:c>=14&&c<26?c-14:c;}
static const char* fighterName(unsigned c){return c==29?"FALCO":c==81?"DK ULT":c==44?"J DK":c==50?"J PIKA":c==45?"E PIKA":c==51?"E SAMUS":c==35?"E LINK":c==39?"J LINK":c==49?"J YOSHI":c==42?"J MARIO":c==40?"J FALCON":c==43?"J LUIGI":c==12?"MASTER HAND":c==13?"METAL MARIO":c==26?"GIANT DK":c>=14&&c<26?"POLYGON":c<12?names[c]:"SELECT";}
...
void nativeBottomDraw(uint16_t* target,const NativeBottomState* s,unsigned fps,unsigned wide,unsigned page,unsigned reports,unsigned errors,unsigned audio){
    ...
    if(page==BOTTOM_CONTROLS)controls();
        ...
        if(s->page==BOTTOM_BATTLE&&s->stage<9)center(160,201,stages[s->stage],7,muted);
            ...
            if(s->scene==16)center(160,201,"TAP FOX, DK OR PIKA CARD TO SWITCH",7,muted);
            else
#endif
...
#ifdef SSB_REMIX_PROBE
        center(160,179,"REMIX FIGHTER TEST / TAP FOX, DK OR PIKA",8,gold);
