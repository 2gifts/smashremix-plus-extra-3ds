...
extern s32 nativeRemixCrowdChantFGM(unsigned fkind);
#endif
...
sb32 ftPublicTryStartCall(GObj *gobj, f32 knockback, s32 player_num)
{
    ...
    {
        s32 fkind = ftGetStruct(fighter_gobj)->fkind;
#ifdef SSB_REMIX_PROBE
        s32 remix_fgm = nativeRemixCrowdChantFGM(fkind);
        sFTPublicCallID = (u16)((remix_fgm >= 0) ? remix_fgm : port_fighter_public_call_fgm(fkind));
#else
        sFTPublicCallID = (u16)port_fighter_public_call_fgm(fkind);
#endif
    }
#else
